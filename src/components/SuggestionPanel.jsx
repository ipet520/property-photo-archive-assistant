...
export default function SuggestionPanel({ suggestion, onApply, onApplyEmpty, onIgnore, compact = false }) {
  ...
  if (compact) {
    return (
      <section className="archive-suggestion-panel compact suggestion-strip">
        <div className="suggestion-strip-main">
          <strong>已生成归档建议</strong>
          <span title={[suggestion.itemName || suggestion.workItem, keywordsText].filter(Boolean).join(' / ')}>
            {[suggestion.itemName || suggestion.workItem, keywordsText].filter(Boolean).join(' / ') || primarySource}
          </span>
        </div>
        <div className="suggestion-strip-actions">
          <button type="button" className="primary" onClick={onApply}>应用建议</button>
          <button type="button" className="ghost" onClick={() => setExpanded((value) => !value)}>{expanded ? '收起详情' : '查看详情'}</button>
          <button type="button" className="ghost" onClick={onIgnore}>忽略</button>
        </div>
        {expanded && (
          <div className="suggestion-strip-detail">
            <p>{suggestion.confidenceText || '建议仅用于辅助填写，归档前请人工确认。'}</p>
